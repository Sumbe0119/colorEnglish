...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const user = await this.prisma.user.create({
      data: {
        ...
        profile: { create: {} }, // хоосон StudentProfile — onboarding-аар бөглөнө
        streak: { create: {} },
  ...
  async refresh(refreshToken: string) {
    ...
    // Rotate: хуучин token-ийг хүчингүй болгоод шинийг олгоно
  ...
  private async issueTokenPair(userId: string, email: string, role: string): Promise<TokenPair> {
    ...
    const refreshTokenValue = randomUUID() + randomUUID(); // энгийн opaque token
