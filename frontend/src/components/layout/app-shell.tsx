...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isRule = pathname.startsWith("/rule") && !isRuleA2 && !isRuleB1;
    ...
    <div className="flex h-dvh overflow-hidden font-body text-mist-50">
        ...
        <nav className="flex-1 space-y-1 p-3">
          <Link
            ...
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
              isStories ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
            }`}
          >
          ...
          <Link
            href="/reading/words"
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
              isMyWords ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
            }`}
          >
          ...
          <Link
            href="/rule"
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
              isRule ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
            }`}
          >
            <BookOpen className="h-4 w-4" />
            A1 түвшний дүрэм
          ...
          <Link
            href="/rule/b1"
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
              isRuleB1 ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
            }`}
          >
            <BookOpen className="h-4 w-4" />
            B1 түвшний дүрэм
          ...
          <Link
            href="/profile"
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
              isProfile ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
            }`}
          >
          ...
          {(user?.role === "ADMIN" || user?.role === "EDITOR") && (
            <Link
              href="/admin"
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${
                isAdmin ? "bg-brand/20 text-brand shadow-glow" : "text-mist-300 hover:bg-ink-800 hover:text-mist-50"
              }`}
            >
        ...
        {!isImmersiveReader && (
          <div className="shrink-0 border-b border-ink-600/80 bg-ink-900/95 backdrop-blur-md">
            <header className="flex items-center gap-3 px-4 py-3 lg:hidden">
              {isStoryOverview ? (
                <Link href="/reading" className="inline-flex min-w-0 items-center gap-2 text-sm text-mist-400 hover:text-mist-50">
            ...
            {isStoryOverview && (
              <div className="hidden border-b border-ink-600/60 px-4 py-2.5 sm:px-6 md:px-10 lg:block lg:px-10">
                <div className="mx-auto max-w-6xl">
                  <Link href="/reading" className="inline-flex items-center gap-2 text-sm text-mist-400 hover:text-mist-50">
      ...
      {mobileMenuOpen && (
        <div
          ...
          aria-modal="true"
        >
          <div className="flex h-full w-[80%] max-w-[300px] flex-col bg-ink-900 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-auto shrink-0 border-t border-ink-600/80 p-4">
              {user && (
                <Link href="/profile" onClick={() => setMobileMenuOpen(false)} className="mb-3 block rounded-lg p-1 hover:bg-ink-800/80">
                  <p className="truncate text-sm font-medium text-mist-100">{user.firstName ?? "Хэрэглэгч"}</p>
                  <p className="truncate text-xs text-mist-400">{user.email}</p>
                </Link>
              )}
