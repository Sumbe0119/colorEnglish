...
type RegisterFormProps = {
  /** Өгвөл /verify-email руу шилжихгүй — onboarding дотор кодыг шууд баталгаажуулна */
...
export function RegisterForm({
  ...
  subtitle = "2 минутын дотор бүртгүүлээд, өөрт тохирсон хичээлээ олоорой.",
}: RegisterFormProps = {}) {
  ...
  const onSubmit = async (values: RegisterFormValues) => {
    ...
    try {
      ...
      if (onRegistered) onRegistered(data.email);
      else router.push(`/verify-email?email=${encodeURIComponent(data.email)}`);
    } catch (err) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: "easeOut" }}>
      ...
      <p className="mt-2 text-sm text-mist-300">{subtitle}</p>
